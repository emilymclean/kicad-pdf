...
for i in "${!file_array[@]}"; do
  ...
  if [[ "$extension" == "kicad_sch" || "$extension" == "sch" ]]; then
    ...
    kicad-cli sch export pdf "$path" -o "/working-tmp/$index.pdf"
    output_pdfs+=("/working-tmp/$index.pdf")
    ...
    for j in "${!copper_layers[@]}"; do
      ...
      if [[ "$copper_layer" == "F" || "$copper_layer" == "B" ]]; then
        ...
        kicad-cli pcb export svg "$path" -o "/working-tmp/$j.svg" -l "Edge.Cuts"
        for k in "${!pcb_layers[@]}"; do
            pcb_layer="${pcb_layers[$k]}"
            kicad-cli pcb export svg "$path" -o "/working-tmp/$j.c.svg" -l "$copper_layer.$pcb_layer" --exclude-drawing-sheet
            python3 /scripts/combine.py "/working-tmp/$j.svg" "/working-tmp/$j.c.svg" "/working-tmp/$j.svg"
        done
        
        rsvg-convert -f pdf -o "/working-tmp/$index-$j.pdf" "/working-tmp/$j.svg"
      else
        kicad-cli pcb export svg "$path" -o "/working-tmp/$j.svg" -l "$copper_layer.Cu,Edge.Cuts"
        rsvg-convert -f pdf -o "/working-tmp/$index-$j.pdf" "/working-tmp/$j.svg"
      fi

      combine_pdfs+=("/working-tmp/$index-$j.pdf")
    done

    if [ ${#extra_pcb_layers[@]} -ne 0 ]; then
      kicad-cli pcb export svg "$path" -o "/working-tmp/x.svg" -l "${extra_pcb_layers[*]},Edge.Cuts"
      rsvg-convert -f pdf -o "/working-tmp/$index-x.pdf" "/working-tmp/x.svg"
      combine_pdfs+=("/working-tmp/$index-x.pdf")
    fi

    # Export PCB to PDF
    pdfunite "${combine_pdfs[@]}" "/working-tmp/$index.pdf"
